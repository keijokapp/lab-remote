...
class Display extends React.Component {
	...
	render() {
		let clipboard;
		if(this.state.sidepane) {
			clipboard = <div id="clipboard-container">
				<h4><label htmlFor="clipboard">Clipboard:</label></h4>
				<textarea autoFocus id="clipboard" ref="clipboard" onBlur={e => setClipboardData(e.target.value)} style={{height: '100%', width: '100%'}} defaultValue={clipboardData} />
				<button style={{width: '100%', padding: '10px', marginTop: '10px'}} onClick={e => sendControlAltDelete()}>Control + Alt + Delete</button>
			</div>;
		}
		...
		if(this.state.connecting) {
			connecting = <div className="overlay">
				<div className="modal">
					<p style={{fontSize: '40px', textTransform: 'uppercase', fontWeight: 'bold'}}>connecting</p>
				</div>
			</div>;
		}

		return <div>
			<div ref="display-container" />
			{clipboard}
			{connecting}
		</div>
	}
...
ReactDOM.render(<Display ref={root} />, document.querySelector('#app'));
